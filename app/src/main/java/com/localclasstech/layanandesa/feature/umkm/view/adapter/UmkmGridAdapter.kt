...
import com.localclasstech.layanandesa.databinding.LayoutUmkmGridBinding
...
class UmkmGridAdapter(
    private var umkmList: List<Umkm> = emptyList(),
    private val onItemClick: (Umkm) -> Unit
) : RecyclerView.Adapter<UmkmGridAdapter.UmkmGridViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): UmkmGridViewHolder {
        val binding = LayoutUmkmGridBinding.inflate(
            LayoutInflater.from(parent.context), parent, false
        )
        return UmkmGridViewHolder(binding)
    }

    override fun onBindViewHolder(holder: UmkmGridViewHolder, position: Int) {
        holder.bind(umkmList[position])
    }

    override fun getItemCount(): Int = umkmList.size

    inner class UmkmGridViewHolder(
        private val binding: LayoutUmkmGridBinding
        ...
        fun bind(umkm: Umkm) {
            with(binding) {
                ...
                // Set kategori background color berdasarkan kategori
